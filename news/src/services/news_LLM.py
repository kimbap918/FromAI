...
def generate_system_prompt(keyword: str, today_kst: str) -> str:
    ...
    prompt = (
        ...
        * 3번 제목: 기사 내용의 핵심 가치나 영향력을 강조하는 제목
        ...
        - 500~800자 내외로 작성 (단, 제공된 기사가 짧으면 불필요한 내용을 추가하지 않는다.)
        - 기사의 흐름과 논점을 유지하고, 의미를 변형하지 않는다.
        - 원문의 단어와 문장 구조를 그대로 사용하지 말고, 동의어와 다양한 표현으로 대체한다.
        - 원문의 모든 주요 사실은 반드시 포함하되, 표현 방식은 완전히 다르게 구성한다.
        - 문단의 순서를 논리적으로 재배치하되, 기사의 흐름은 자연스럽게 유지한다.
