...
def build_stock_prompt(today_kst):
    ...
    "[Special Rules for Stock-Related News]\n"
        ...
        f"   - 특수문자·어휘 규칙: 제목과 동일한 금지 목록을 따른다(괄호 포함 금지), 등락 표시는 수치+‘상승/하락’만 허용하며 ‘플러스/마이너스/강세/약세’와 ± 기호는 사용하지 않는다.\n"
        ...
        f"   - 본문에 날짜가 등장하거나 사용해서는 안 됨.\n"
        f"   - **시계열 추론 금지: '이후/뒤/한때/장중/다음' 등 시간 암시어 금지.**\n"
        f"   - **시가·고가·저가는 연결어·순서표현('이후/뒤/한때/장중/다음') 없이 반드시 ‘나열/범위’로만 제시하고, 현재가는 범위 내 ‘위치’만 기술.**\n"
