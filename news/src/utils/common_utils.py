...
def build_stock_prompt(today_kst):
    ...
    "[Special Rules for Stock-Related News]\n"
        ...
        f"2. 제목(절대 규칙)\n"
        f"   - 국내주식 출력 형식과 순서(고정): '키워드' 주가, {title_time_format} '내용'\n" 
        f"   - 해외주식 출력 형식과 순서(고정): '키워드' 주가, '내용' \n"
        ...
        f"   - 상한가 이탈 마감 시, '장중 상한가' 표현을 **가격/등락률보다 앞에 배치**하여, 반드시 '...장중 상한가, O,OOO원 OO% 상승 마감' 순서로 작성할 것(상한가 마감 오인 방지).\n\n"
        f"3. 본문(절대 규칙)\n"
