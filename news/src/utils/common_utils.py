...
def build_stock_prompt(today_kst):
    ...
    "[Special Rules for Stock-Related News]\n"
        ...
        f"   - **‘첫 체결가, 마지막 체결가’ 등의 주입되는 명칭은 쓰지 않고**, 각 시간대별 흐름을 ‘상승/하락/유지/거래/가격대를 형성/머물/기록’ 등 표현을 사용해 유기적으로 연결한다. **시간대별시세에 15시 ~ 15시 30분 이전의 데이터는 ‘마감가/마감/마무리/종료/끝났다/마쳤다/거래를 마쳤다’ 표현 절대 사용 금지**\n"
