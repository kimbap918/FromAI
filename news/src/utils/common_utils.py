...
def build_stock_prompt(today_kst):
    ...
    "[Special Rules for Stock-Related News]\n"
        f"1. 제목\n"
        f"   - 출력 형식과 순서(고정): '키워드', {title_time_format} '내용'\n" 
        f"   - 키워드 뒤에는 항상 콤마(,)를 붙일 것.\n"
        f"   - 제목 모두 위 형식을 유지하고 가격+등락률(%) 포함한 '내용'만 다르게 작성할 것.\n"
        f"   - 금액은 천단위 콤마, 가격+등락률(%) 함께 표기.\n"
        f"   - 신규상장 값이 True 일 경우, IPO 사실과 가격을 핵심에 배치.\n"
        f"   - 장 마감인 경우에만 문장 끝을 \"<상태> 마감\"으로 마무리.\n\n"
        f"2. 본문(절대 규칙)\n"
        f"   - 시스템이 자동 넣는 날짜·시간·출처·\"… 기준\" 문구와 날짜 언급을 반복하지 않음.\n"
        f"   - 종목명으로 시작하고 [주식 정보]만으로 객관 서술.\n"
        f"   - [주식 정보]를 최대한 활용하여 본문 작성(추론 금지).\n"
        f"   - {title_time_format}은 제목 전용 슬롯이며, 본문에 등장하거나 사용해서는 안 됨."
        f"   - **시계열 추론 금지: '이후/뒤/한때/장중/다음' 등 시간 암시어 금지.**\n"
        f"   - 시가·고가·저가는 연결어·순서표현 없이 ‘나열/범위’로만 제시하고, 현재가는 범위 내 ‘위치’만 기술.\n"
        f"   - 변동폭(고가-저가)은 한 문장으로 산출·표시하되, 시간적 흐름을 암시하지 않는다.\n"
        f"   - N/A 값은 생략, 범위·수준 등 드러난 사실만 사용(추론 금지).\n"
        f"   - 의미 부여는 ‘비교’로만 하며, 시가총액·순위, 업종 등락률 대비, PER vs 업종 PER, 외국인 소진율 중 최소 1개 이상을 반드시 포함한다(추측·전망 금지).\n"
        f"   - 신규상장 값이 True 일 경우, '지난 종가'=공모가로 간주해 서술.\n"
        f"   - 감정·권유·주관 어휘 금지: 투자자들, 관심, 주목, 기대, 풀이, 분석 등.\n\n"
        f"3. [해외주식 정보]가 있는 경우 추가 규칙\n"
        f"   - 제목에는 날짜 미포함.\n"
        f"   - us_time으로 장 중/장 마감 구분(표현: 장 중, 장 마감).\n"
        f"   - 정규장 서술 뒤 시간 외 거래를 별도 문장으로 덧붙임.\n\n"
        ...
        f"5. [General Style Standards]\n"
        f"   - 반드시 장 시작/장중/장 마감 시점에 따라 서술 시제를 조정할 것.\n"
        f"   - **'전일/전날/전 거래일' 대신 '지난 종가'만 사용.**\n"
        f"   - 단순 데이터 나열을 금지하며, 원인과 결과를 엮어 [News Generation Process] 기반으로 구성할 것.\n"
        f"   - 본문 특수문자·어휘 규칙: 제목과 동일한 금지 목록을 따른다(괄호 포함 금지), 등락 표시는 수치+‘상승/하락’만 허용하며 ‘플러스/마이너스/강세/약세’와 ± 기호는 사용하지 않는다.\n"
