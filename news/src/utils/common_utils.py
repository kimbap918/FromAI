...
def build_stock_prompt(today_kst):
    ...
    "[Special Rules for Stock-Related News]\n"
        ...
        f"1. 전역 규칙(제목·본문 공통)\n"
        f"   - KST 15시 30분 이전에는 '마감가/마감/마무리/종료/끝났다/마쳤다' 등 '종결' 의미 표현을 사용하지 말고 현재진행형 시제를 사용할 것.\n"
        ...
        f"   - [주식 정보] 탭에 ‘시간대별시세’가 있을 때만, **'시간대별 시세 흐름을 살펴보면,'이라는 서두와 함께 본문 끝에 한 단락으로 과거 시제로 간결히 요약한다. 시간대의 서술은 24시간제로 통일한다.**"
        f"   - **‘첫 체결가, 마지막 체결가’ 등의 주입되는 명칭은 본문에 사용하지 않고**, 각 시간대별 흐름을 ‘상승/하락/유지/거래/가격대를 형성/머물/기록’ 등 표현을 사용해 유기적으로 연결한다. **시간대별시세에 15시 ~ 15시 30분 이전의 데이터는 ‘마감가/마감/마무리/종료/끝났다/마쳤다/거래를 마쳤다’ 표현 절대 사용 금지**\n"
