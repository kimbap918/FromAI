# -*- coding: utf-8 -*-
"""
DCInside 댓글 생성 (Gemma3 버전, 단계별 로깅, 핵심 문장 무작위 추출)

⚡ 전체 프로세스 (LLM 2회 호출):
  1. 게시글의 키워드 추출 (LLM 1회)
  1.5. 게시글 요약 생성 (감정 반영)
  2. 해당 게시글의 ±N개의 이웃 글 수집
  2.5. 키워드 기반 검색으로 관련 글 추가 수집
  3. 유사 키워드를 가진 글의 데이터 추출
  4. 해당 키워드를 가진 글들을 묶어서 데이터 정리
  4.5. 모든 관련 글에서 핵심 문장 후보군을 뽑아 N개 무작위 추출 (로직 변경)
...
def _make_chat():
    ...
    def chat(messages, *, model=DEFAULT_MODEL, temperature=0.9, max_tokens=300, timeout=60, retry=1):
        """
        LLM 대화 함수
...
# STEP 1: 키워드 + 요약 추출
# -----------------------------
def analyze_post(title: str, body: str, top_k: int = 5):
    """
    ...
    try:
        ...
        try:
            ...
            logger.error("[STEP 1] Fallback 실패: konlpy가 설치되지 않았습니다. 키워드 추출을 건너뜁니다.")
            keywords = []
            summary = (body or "")[:50] + ("…" if len(body or "") > 50 else "")
        except Exception as konlpy_e:
            logger.error(f"[STEP 1] Fallback 실패: konlpy 오류 발생: {konlpy_e}")
            keywords = []
            summary = (body or "")[:50] + ("…" if len(body or "") > 50 else "")
...
def filter_by_keywords(neighbors, keywords):
    """
    [STEP 3 & 4] 수집된 글 목록의 중복을 제거하고, 키워드와 연관된 글만 필터링하여 묶는다.

    Args:
        neighbors (list): (no, title, body) 튜플 리스트 (이웃글 + 검색글)
        keywords (list[str]): 핵심 키워드 리스트

    Returns:
        list of (no, title, body): 키워드가 매칭된 관련 글 리스트
    """
    logger.info("[STEP 3] 키워드 필터링 및 중복 제거 시작")
    ...
    if len(neighbors) != len(unique_pool):
        logger.info(f"[STEP 3] 중복 글 제거: {len(neighbors)}건 -> {len(unique_pool)}건")

    # 키워드 필터링
    filtered = []
    for no, t, b in unique_pool:
        content = (t or "") + " " + (b or "")
        if any(kw and kw in content for kw in keywords):
            filtered.append((no, t, b))
            
    logger.info(f"[STEP 4] 최종 필터링된 글 수: {len(filtered)}")
    return filtered

# ---------------------------------
# STEP 4.5: 핵심 문장 추출 (신규)
# ---------------------------------
def extract_top_sentences(related_posts, keywords, top_n=3, pool_size=10):
    """
    모든 관련 글에서 키워드가 포함된 핵심 문장을 추출하고, 그 중 일부를 무작위로 선택한다.

    Args:
        related_posts (list): (no, title, body) 튜플 리스트
        keywords (list[str]): 원본 글의 핵심 키워드
        top_n (int): 최종 선정할 문장 개수
        pool_size (int): 상위 후보군으로 사용할 문장 개수

    Returns:
        list: 점수가 높은 상위 N개의 문장(str) 리스트
    """
    logger.info(f"[STEP 4.5] 핵심 문장 추출 시작 (상위 {pool_size}개 중 {top_n}개 무작위 선별)")
    
    all_sentences = []
    for _, t, b in related_posts:
        content = (t or "") + " " + (b or "")
        # 간단한 문장 분리. 마침표, 물음표, 느낌표, 또는 줄바꿈 기준.
    ...
    for sentence in all_sentences:
        ...
        if score > 0: # 키워드가 하나라도 있는 문장만 후보로
            scored_sentences.append((sentence, score))

    # 점수가 높은 순, 점수가 같으면 문장 길이가 짧은 순으로 정렬 (간결한 문장 우선)
    scored_sentences.sort(key=lambda x: (x[1], -len(x[0])), reverse=True)
    
    # 상위 pool_size 만큼을 후보군으로 선택
    candidate_pool = [sentence for sentence, score in scored_sentences[:pool_size]]

    # 후보군이 top_n 보다 적으면 그대로 사용, 아니면 무작위 샘플링
    if len(candidate_pool) <= top_n:
        selected_sentences = candidate_pool
    else:
        selected_sentences = random.sample(candidate_pool, top_n)

    logger.info(f"[STEP 4.5] 핵심 문장 선별 완료")
    for sentence in selected_sentences:
        logger.info(f"[STEP 4.5]  → {sentence[:60]}...")
...
# STEP 5: 댓글 생성
# -----------------------------
def generate_comment(title: str, body: str, top_sentences, keywords, summary: str, search_keywords):
    """
    ...
    Args:
        title (str): 게시글 제목
        body (str): 게시글 본문
        top_sentences (list): 선별된 핵심 문장 리스트
    ...
    for sentence in top_sentences:
        sanitized_sentence = _sanitize_for_llm(sentence, 200)
        neigh_s.append(f"- {sanitized_sentence}")
    ...
    base_prompt = (
        ...
        "본문 요약과 키워드를 반영하고, 주변 글들의 핵심 내용에 공감할 것.\n"
    ...
    out = chat(
        ...
        model=DEFAULT_MODEL, temperature=0.9, max_tokens=200,
    )
...
# -----------------------------
def main():
    parser = argparse.ArgumentParser(description="DCInside 게시글 기반 댓글 생성 (핵심 문장 무작위 추출)")
    parser.add_argument("--gid", default="youngwoong", help="갤러리 ID")
    parser.add_argument("--no", type=int, required=True, help="게시글 번호")
    parser.add_argument("--min-len", type=int, default=10, help="본문 최소 글자 수")
    parser.add_argument("--neighbors", type=int, default=5, help="±N 범위 내 이웃 글 수집 개수")
    parser.add_argument("--top-n", type=int, default=3, help="참고할 핵심 문장 개수")
    parser.add_argument("--pool-size", type=int, default=10, help="핵심 문장 추출 시 사용할 상위 후보군 개수")
    ...
    search_hits, search_keywords = search_gallery_by_keywords(args.gid, keywords, per_kw=5, pages=1)
    ...
    # STEP 4.5: 핵심 문장 추출
    top_sentences = extract_top_sentences(related, keywords, top_n=args.top_n, pool_size=args.pool_size)

    # STEP 5: 댓글 생성
    comment = generate_comment(title, body, top_sentences, keywords, summary, search_keywords)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        # 기본 테스트용 파라미터
        sys.argv.extend(["--gid", "swimming", "--no", "372468"])
    main()


    ## ["--gid", "swimming", "--no", "372763"]
    ## ["--gid", "songgain", "--no", "21029"]
    ## ["—gid", "youngtak", "—no", "209571"]
    ## ["--gid", "youngwoong", "--no", "355289"]
